...
/**
 * English reference dictionary for the docs browser-client page. Copy
 * moved verbatim from `app/[locale]/docs/web/page.tsx`.
 */
export const docsWeb: DocsWebDict = {
  ...
  overviewSeg1:
    " opens Codewhale's embedded browser client over the canonical Runtime API. It is a local surface: the server always binds to ",
  overviewSeg2:
    ", cannot be rebound to a LAN address, and cannot run with Runtime authentication disabled. The default address is ",
  overviewSeg3: "; on a port collision, pick another loopback port with ",
  overviewSeg4: ". Stop the process with Ctrl+C and the browser session ends with it.",
  overviewLead2:
  ...
  localSeg1: " accepts only ",
  localSeg2: " — there is no ",
  localSeg3:
    " and no insecure-auth option on this command. Do not treat it as a public website or expose its port through router forwarding, a public reverse proxy, or a tunnel. The separate ",
  localSeg4: " and ",
  localSeg5:
    " modes carry different deployment and authentication contracts; read the Runtime API documentation before operating either one, especially before selecting a non-loopback bind.",
