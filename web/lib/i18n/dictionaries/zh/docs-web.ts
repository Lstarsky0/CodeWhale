...
/**
 * Simplified-Chinese dictionary for the docs browser-client page. Copy
 * moved verbatim from the former `isZh` branches in
 * `app/[locale]/docs/web/page.tsx`.
 */
export const docsWeb: DocsWebDict = {
  ...
  overviewSeg1:
    " 在 canonical 运行时 API 之上打开 Codewhale 内嵌的浏览器客户端。它是一个纯本地界面：服务器始终绑定 ",
  overviewSeg2: "，无法改绑到局域网地址，也无法在关闭运行时认证的情况下运行。默认地址是 ",
  overviewSeg3: "；端口冲突时用 ",
  overviewSeg4: " 换一个回环端口。Ctrl+C 停止进程，浏览器会话随之结束。",
  overviewLead2:
  ...
  localSeg4: " 和 ",
  localSeg5: " 模式有不同的部署与认证约定，操作它们（尤其是选择非回环绑定）之前请阅读运行时 API 文档。",
